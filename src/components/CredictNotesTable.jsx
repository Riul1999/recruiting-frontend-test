...
function CredictNotesTable({tableTitle, selectedInvoice, setSelectedCreditNote}){
        ...
        <table className="table-auto text-center rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            {!loading && <tbody>
                {invoices.map(invoice => {
                    ...
                    return (invoice.type === "credit_note" && invoice.reference === selectedInvoice) && 
                    <tr className="bg-white border dark:bg-gray-800 dark:border-gray-700">
                        ...
                        {(invoice.currency === "CLP") ?
                            <TableRow boldText={`$${invoice.amount}${invoice.currency}`} normalText={`($${parseInt(anotherAmount)}${anotherCurrency})`}/> :
                            <TableRow boldText={`$${anotherAmount}${anotherCurrency}`} normalText={`($${invoice.amount}${invoice.currency})`}/>
                        }
                        <TableRow boldText={''} normalText={invoice.reference}/>
                    </tr>
                })}
            </tbody>}
        </table>
...
export default CredictNotesTable